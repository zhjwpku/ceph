    ...
    case STATE_CONNECTING_WAIT_IDENTIFY_PEER:
      {
        ...
        ::encode(async_msgr->get_myaddr(), myaddrbl, async_msgr->local_features);  // not sure
        r = try_send(myaddrbl);
        if (r == 0) {
    ...
    case STATE_ACCEPTING:
      {
        ...
        ::encode(async_msgr->get_myaddr(), bl, async_msgr->local_features);  // not sure
        ...
        ::encode(socket_addr, bl, connect_msg.features);  // not sure
